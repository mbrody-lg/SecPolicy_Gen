...
def _set_common_env(monkeypatch):
    """Provide the minimum env needed to create the policy app in tests."""
    monkeypatch.setattr(app_module, "load_dotenv", lambda: None)
    monkeypatch.setenv(
        "CONFIG_PATH",
        str(Path(__file__).resolve().parents[1] / "app" / "config" / "policy_agent.yaml"),
    )
    monkeypatch.setenv("MONGO_URI", "mongodb://mongo:27017/policydb")
