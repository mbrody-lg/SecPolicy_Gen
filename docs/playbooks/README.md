...
When the work needs a single CI-aligned evidence command for the full critical loop, use `make critical-path-validation`. It runs the service suites for `context-agent`, `policy-agent`, and `validator-agent`, then the repository governance suite, then resets into the Docker smoke run.
...
- [Local Application Validation](./local-application-validation.md)
